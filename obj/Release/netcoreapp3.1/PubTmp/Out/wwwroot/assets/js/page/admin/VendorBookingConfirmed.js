$(function () {
    VendorloadBookingsConfirmed();
});

function VendorloadBookingsConfirmed() {
    $.ajax({
        type: "Post",
        contentType: "application/json;charset=utf-8",
        url: "/Dashboard/VendorloadBookingsConfirmed",
        dataType: "json",
        success: function (response) {
            var dat = response;
            if (dat.length > 0) {
                var table = $('#tbl_VendorBookingConfirmed').DataTable({ destroy: true });
                table.destroy();
                $('#tbl_VendorBookingConfirmed').DataTable({
                    data: dat,
                    autoWidth: true,
                    responsive: true,
                    columns: [
