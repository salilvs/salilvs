...
function loadBookingsConfirmed() {
    $.ajax({
        type: "Post",
        contentType: "application/json;charset=utf-8",
        url: "/Dashboard/loadBookingsConfirmed",
        dataType: "json",
        success: function (response) {
            var dat = response;
            if (dat.length > 0) {
                var table = $('#tbl_BookingsConfirmed').DataTable({ destroy: true });
                table.destroy();
                $('#tbl_BookingsConfirmed').DataTable({
                    data: dat,
                    autoWidth: true,
                    responsive: true,
                    columns: [
