$(function () {
    VendorloadBookingsCancelled();
});

function VendorloadBookingsCancelled() {
    $.ajax({
        type: "Post",
        contentType: "application/json;charset=utf-8",
        url: "/Dashboard/VendorloadBookingsCancelled",
        dataType: "json",
        success: function (response) {
            var dat = response;
            if (dat.length > 0) {
                var table = $('#tbl_vendorBookingCancelled').DataTable({ destroy: true });
                table.destroy();
                $('#tbl_vendorBookingCancelled').DataTable({
                    data: dat,
                    autoWidth: true,
                    responsive: true,
                    columns: [
