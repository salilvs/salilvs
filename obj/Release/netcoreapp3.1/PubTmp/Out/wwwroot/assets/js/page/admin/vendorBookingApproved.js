$(function () {
    vendorBookingApproved();
});

function vendorBookingApproved() {
    $.ajax({
        type: "Post",
        contentType: "application/json;charset=utf-8",
        url: "/Dashboard/VendorloadBookingsApproved",
        dataType: "json",
        success: function (response) {
            var dat = response;
            if (dat.length > 0) {
                var table = $('#tbl_vendorBookingApproved').DataTable({ destroy: true });
                table.destroy();
                $('#tbl_vendorBookingApproved').DataTable({
                    data: dat,
                    autoWidth: true,
                    responsive: true,
                    columns: [
